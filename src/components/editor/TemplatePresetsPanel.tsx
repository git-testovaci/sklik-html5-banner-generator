...
function firstEditableSelection(state: BannerEditorState): SelectedLayer {
  ...
  return { type: "asset", id: layer.assetId ?? layer.id };
}
  ...
  return (
    <section className="rounded-xl border border-zinc-800/80 bg-zinc-900/40">
      <div className="border-b border-zinc-800/60 px-4 py-3">
        <h2 className="text-sm font-medium text-zinc-300">Banner templates</h2>
        <p className="mt-1 text-xs text-zinc-500">
          Storyboard templates replace scenes and motion while keeping uploaded assets.
        </p>
      </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                {items.map((template) => (
                  <button
                    ...
                    className={`rounded-lg border px-3 py-2.5 text-left transition-colors hover:bg-violet-950/30 ${
                  ...
                  >
                    ...
                    <p className="mt-1 text-[10px] text-zinc-600">
                      {template.sceneCount} scenes · {Math.round(template.totalDurationMs / 1000)}s ·{" "}
                      {template.keyEffects.slice(0, 3).join(", ")}
                    </p>
                    <p className="mt-0.5 text-[10px] text-zinc-600">{template.useCase}</p>
                  </button>
                ))}
              </div>
            </div>
